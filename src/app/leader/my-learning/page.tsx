...
const LeaderLearning = () => {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      const saved = localStorage.getItem("leader_selectedScriptures");
      if (saved) {
  ...
  const displayScriptures =
    selectedScriptures.length > 0
      ? scriptures.filter((s) => selectedScriptures.includes(s.id))
      : scriptures;
